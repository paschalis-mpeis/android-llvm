...
public class TestVecShort {
  ...
  private static void expectEquals(int expected, int  result, String action) {
    ...
    String msg = "Expected: " + expected + ", found: " + result + " for " + action;
    msg = className + ": " + msg;
    if (expected != result) {
      CheckTest.RunAndroidTestFailed(msg);
      // throw new Error("Expected: " + expected + ", found: " + result + " for " + action);
    } else {
      CheckTest.RunAndroidTestPassedSilent(msg);
    }
  }
