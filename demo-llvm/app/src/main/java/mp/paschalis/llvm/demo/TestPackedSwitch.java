...
public class TestPackedSwitch {
  ...
  public static final String info = TestPackedSwitch.class.getSimpleName();

  // Simple packed-switch starting at a negative index.
  public static String packedSwitch2(int value) {
    switch (value) {
      case -3:
        return "-3";
        // Log.d(TAG, info+ "-3"); break;
      case -2:
        return "-2";
        // Log.d(TAG, info+ "-2"); break;
      case -1:
        return "-1";
        // Log.d(TAG, info+ "-1"); break;
      case 0:
        return "0";
        // Log.d(TAG, info+ "0"); break;
      case 1:
        return "1";
        // Log.d(TAG, info+ "1"); break;
      case 2:
  ...
  public static int Int_Glob = 34;

  /**
   * An example from Dhrystone
   *
   * @param Enum_Par_Val
   * @param Enum_Par_Ref
   */
  static void PackedSwitchDhry(int Enum_Par_Val, int Enum_Par_Ref[]) {
    Enum_Par_Ref[0] = Enum_Par_Val;
    if (!Func_3(Enum_Par_Val))
      Enum_Par_Ref[0] = Dhry.Ident_4;
    switch (Enum_Par_Val) {
      case Dhry.Ident_1:
        Enum_Par_Ref[0] = Dhry.Ident_1;
        break;
      case Dhry.Ident_2:
        // Check if no PackedSwitch generated, make it parameter
        if (Int_Glob > 100)
          Enum_Par_Ref[0] = Dhry.Ident_1;
        else
          Enum_Par_Ref[0] = Dhry.Ident_4;
        break;
      case Dhry.Ident_3:
        Enum_Par_Ref[0] = Dhry.Ident_2;
        break;
      case Dhry.Ident_4:
        break;
      case Dhry.Ident_5:
        Enum_Par_Ref[0] = Dhry.Ident_3;
        break;
    }
  }

  private static boolean Func_3(int enum_par_val) {
    if(enum_par_val == 5)
      return false;
  ...
  static long GetPackedSwitchDhryRes(int a[]){
    long sum = 0;
  ...
  public static void Run() {
    Log.d(TAG, "TestPackedSwitch");
    int [] a = new int[10];
    for(int i=0; i<a.length; i++) {
      a[i]=5+(2*1);
    }
    PackedSwitchDhry(10, a);
    // Log.d(TAG, info + "Result: " + GetPackedSwitchDhryRes(a));
    CheckTest.Run(info+": packedSwitchDhry"," " + GetPackedSwitchDhryRes(a), GetPackedSwitchDhryRes(a) == 73);

    String result = "";
    for (int i = -2; i < 8; i++) {
      result+=""+packedSwitch2(i);
    }
    result+="_"+packedSwitch2(Integer.MIN_VALUE);
    result+="_"+packedSwitch2(Integer.MAX_VALUE);
    CheckTest.Run(info+": packedSwitch2"," " + result, result.equals("-2-10123i456default_default_default"));
