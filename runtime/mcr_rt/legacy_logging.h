...
#error "legacy_logging not in use"
...
class LogMessage {
 public:
#ifdef ART_MCR_TARGET
  LogMessage(const char* file, unsigned int line, LogSeverity severity, int error, McrLog mcr_log)
      : LogMessage(file, line, severity, error) {
    mcr_log_ = mcr_log;
  } 

  static void ReviveLogging();
#endif
...
#ifdef ART_MCR_TARGET
  static void LogLine(const char* file, unsigned int line, LogSeverity severity, const char* msg, McrLog mcr_log);
#else
